import numpy as np
import matplotlib
from utils.general import argmin
...
class HolesFilling(object):
    ...
    def fill_holes_KNN(self, input_with_hole, K=10):
        """
        ...
        y_x_bounds_indeces = np.argwhere(self.bounds_mask).tolist() # bounds's pixels coordinates
        # For each pixel in hole:
        # C.1. compute the weigths of this pixel from K boundaries
        # C.2 compute the dot product of weights with boundaries intensions
        # C.3. normalize the former result by the sum of weights.
        # C.4. Assign the normalized result to be new intensity
        for i, (uy, ux) in enumerate(y_x_hole_indeces):
            # C.1 build weights array
            K_weights = np.zeros(K)
            K_boundary_intensities = np.zeros(K)
            for i in range(K):
                index = argmin(y_x_bounds_indeces, key=lambda vy_vx: self.weight_function((ux,uy), vy_vx))
                K_weights[i] = self.weight_function((ux,uy), y_x_bounds_indeces[index])
                K_boundary_intensities[i] = boundary_intensities[index]
            # C.2.  compute the new value of the hole's pixel
            nominator = np.dot(K_boundary_intensities, K_weights)
            descriminator = np.dot(np.ones(K_weights.shape), K_weights)
            # C.3. Normalization
            new_intensity =  nominator / descriminator # new value in the hole
            # C.4. Assign new intensity
            output[uy, ux] = new_intensity # assignment
